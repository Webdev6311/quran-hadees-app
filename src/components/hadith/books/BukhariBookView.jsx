...
const BukhariBookView = ({
  ...
  if (!selectedChapter) {
    return <p className="page-sub">Book selected. Now choose a chapter from sidebar.</p>;
  }

  if (selectedChapter && !selectedHadith) {
    return <p className="page-sub">{selectedChapter} selected. Pick a hadith number from sidebar.</p>;
  }
  ...
  return (
    <div className="verse-block hadith-card">
      ...
      <div className="hadith-meta">
        <strong>Narrator:</strong> {hadithDetail.narrator || "N/A"}
        {" | "}
        <strong>Grade:</strong> {hadithDetail.grade || "N/A"}
      </div>
    </div>
  );
};

export default BukhariBookView;
